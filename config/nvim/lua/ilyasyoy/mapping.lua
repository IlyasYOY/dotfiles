...
vim.keymap.set("n", "<leader>tp", function()
    ...
    if string.find(cwf, "_test%.go$") then
        vim.cmd("!go test " .. core.current_working_file_dir())
    end
end, { desc = "run test for a package", silent = true })
...
end, { desc = "run test for a file", silent = true })
...
vim.keymap.set("n", "<leader>tf", function()
    ...
    if string.find(cwf, "_test%.go$") then
        ...
        elseif string.match(function_name, "^Test.+") then
            vim.cmd("!go test " .. cwf .. " -run " .. function_name)
        else
