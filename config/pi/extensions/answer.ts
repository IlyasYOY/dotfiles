...
function isOpenAICodexModel(model: Model<Api>): boolean {
	return model.provider === OPENAI_CODEX_PROVIDER;
}

/**
 * Prefer Codex mini for extraction when available, otherwise use the current OpenAI Codex model.
 */
async function selectExtractionModel(
	currentModel: Model<Api>,
	modelRegistry: ModelRegistry,
): Promise<Model<Api> | null> {
	const codexModel = modelRegistry.find(OPENAI_CODEX_PROVIDER, CODEX_MODEL_ID);
	if (codexModel) {
	...
	if (!isOpenAICodexModel(currentModel)) {
		return null;
	}
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			// Select the best OpenAI Codex model for extraction.
			const extractionModel = await selectExtractionModel(ctx.model, ctx.modelRegistry);
			if (!extractionModel) {
				ctx.ui.notify(
					"answer requires an OpenAI Codex model. Run /login for ChatGPT Plus/Pro (Codex) and select a Codex model via /model.",
					"error",
				);
				return;
			}
